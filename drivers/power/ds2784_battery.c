...
static void ds2784_program_alarm(struct ds2784_device_info *di, int seconds)
{
	...
	android_alarm_start_range(&di->alarm, next, ktime_add(next, slack));
}
...
static int ds2784_battery_probe(struct platform_device *pdev)
{
	...
	di->monitor_wqueue = create_freezable_workqueue(
		dev_name(&pdev->dev));
	...
	wake_lock_init(&di->work_wake_lock, WAKE_LOCK_SUSPEND,
			"ds2784-battery");
	android_alarm_init(&di->alarm, ANDROID_ALARM_ELAPSED_REALTIME_WAKEUP,
